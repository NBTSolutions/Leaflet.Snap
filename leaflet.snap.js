...
L.Handler.MarkerSnap = L.Handler.extend({
  ...
  initialize: function (map, marker, options) {
    ...
    function computeBuffer() {
      ...
      // PixelSize (degrees) = C*cos(y)/2^(z+8)
      // math from http://wiki.openstreetmap.org/wiki/Zoom_levels
      var pixelSize = 360 * Math.cos(y * 0.017) / Math.pow(2, z + 8);
      this._buffer = pixelSize * this.options.snapDistance;
    }
